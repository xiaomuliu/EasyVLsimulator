...
public:
	...
	std::string get_logic_status() const;
...
public:
	...
	virtual bool get_tris_enable(){return true;}
	virtual bool get_tris_output(){return false;}
	
	virtual std::string get_address()
	{std::string empty; return empty;}
	virtual std::string get_word(std::string&)
	{std::string empty; return empty;}
	virtual bool read_rom(std::string&){return true;}
}; // class gate
...
class tris: public gate {
	bool enable; //false means output will be "Z"
	bool output; // the 2 value boolean output
	bool validate_structural_semantics();
public:
	tris(std::string name)
		: gate("tris", name){} //ctor
	bool get_tris_enable();
	bool get_tris_output();
}; // class tris
/************************************/
