...
/*
std::string net::get_logic_status() const{
	return logic_value.first;
}
*/
...
bool net::retrieve_from_driver() {
	for(std::list<pin_info>::const_iterator i = connections_.begin();
		...
		if(i->first->isoutput()){//the net should connect to an output pin
			...
			else if(g->get_type() == "tris"){
				if(g->get_tris_enable()){
					return g->get_tris_output();
				}
				else{ continue; } //output of tris is "Z"				
			}
			else if(g->get_type() == "lut"){
				std::string word = g->get_word(g->get_address());
				std::vector<net *> nets_ = i->first->get_nets();				
				for(size_t i = 0; i != nets_.size(); ++i){
					//find corresponding bit
					if(nets_[i]->get_name() == this->get_name()){			
						if(word[nets_.size()-1-i] == '1'){ return true; }
						else{ return false; }
					}
				}
			}
...
bool netlist::create_gate(const evl_component &c) {
	...
	if (c.type == "dff") {
		...
		gate *g = new tris(c.name);
		gates_.push_back(g);
	}
	else if (c.type == "lut") {
		gate *g = new lut(c.name);
		gates_.push_back(g);
	}
...
bool lut::validate_structural_semantics() {
	if (pins_.size() != 2) return false;
	pins_[0]->set_as_output();
	pins_[1]->set_as_input();
}
...
bool tris::get_tris_enable(){	
	net *enable_net = pins_[2]->get_net(0);
	enable = enable_net->retrieve_logic_value();
	return enable;
}

bool tris::get_tris_output(){
	net *in_net = pins_[1]->get_net(0);
	if(enable){
		output = in_net->retrieve_logic_value();
	}	
	return output;
}

std::string lut::get_address(){
	std::string address;
	for(int i = 0; i != pins_[1]->get_nets().size(); ++i)
	{
		net *input_net = pins_[1]->get_nets()[i];
...
std::string lut::get_word(std::string &address){
	//must be in look-up table
	assert(lookup.find(address) != lookup.end());
	return lookup[address];
}

bool lut::read_rom(std::string &evl_file){
	std::string filename = evl_file + ".rom";
	std::ifstream rom_file(filename.c_str());
	if(!rom_file){
		std::cerr << "Can't read the rom file " <<
		filename << std::endl;
		return false;
	}

	std::string line;
	for (int line_no = 1; std::getline(rom_file, line); ++line_no) {
		if(line_no == 1){
			//check matching
		}
		else{//from the 2nd line
			std::string address = dec2bin(line_no-2, pins_[1]->get_nets().size());
			std::string word = dec2bin(str2num<int>(line, std::hex),
					(pins_[0]->get_nets()).size());
			lookup[address] = word;
		}
	}
	return true;
...
{
	//before the simulation, read in the look-up table
	for(std::list<gate *>::const_iterator j = gates_.begin();
		j != gates_.end(); ++j){
		if((*j)->get_type() == "lut"){
			if (!(*j)->read_rom(evl_file)){
				return false;
			}
		}
	}
